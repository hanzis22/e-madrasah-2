<?php 
require '../templates/header_guru.php'; 
require '../templates/sidebar_guru.php';

/** @var mysqli $conn */
 $id_guru = $_SESSION['id'];

// ========== LOGIKA UPLOAD (DIPINDAH KE ATAS) ==========
 $pesan = '';
 $pesan_tipe = '';
...
if (isset($_POST['simpan_profil'])) {
    ...
    $cek_username = mysqli_query($conn, "SELECT id FROM users WHERE username='$username_baru' AND id != '$id_guru'");
    if (mysqli_num_rows($cek_username) > 0) {
        $pesan = 'Username sudah digunakan orang lain!';
        $pesan_tipe = 'danger';
    } else {
        // ✅ 1. PROSES UPLOAD FOTO
        $nama_foto = $_POST['foto_lama']; // default pakai yang lama
        ...
        if (!empty($_FILES['foto']['name'])) {
            ...
            elseif ($ukuran > 2 * 1024 * 1024) {
                ...
                // Buat nama unik
                $nama_foto_baru = 'profil_' . $id_guru . '_' . time() . '.' . $tipe;
        ...
        if (empty($pesan)) {
            if (empty($pass_baru)) {
                mysqli_query($conn, "UPDATE users SET nama='$nama_baru', username='$username_baru', foto='$nama_foto' WHERE id='$id_guru'");
            } else {
                $pass_md5 = md5($pass_baru);
                mysqli_query($conn, "UPDATE users SET nama='$nama_baru', username='$username_baru', password='$pass_md5', foto='$nama_foto' WHERE id='$id_guru'");
            }
...
// Ambil data guru terbaru
 $data_guru = mysqli_query($conn, "SELECT * FROM users WHERE id='$id_guru'");
 $guru = mysqli_fetch_assoc($data_guru);

// Tentukan path foto
 $folder_foto = '../uploads/profil/';
 if (!empty($guru['foto']) && file_exists($folder_foto . $guru['foto'])) {
     $src_foto = '../download.php?f=profil/' . rawurlencode($guru['foto']); // H-04 FIX
} else {
    ...
    <div class="box-putih">
        ...
        <form method="post" action="" enctype="multipart/form-data" style="max-width: 500px;">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="username" class="form-control form-control-sm" value="<?= htmlspecialchars($guru['username']) ?>" required>
                <small class="text-muted">Username digunakan untuk login.</small>
            </div>
            
            <div class="mb-3">
                <label class="form-label">Nama Lengkap</label>
                <input type="text" name="nama" class="form-control form-control-sm" value="<?= htmlspecialchars($guru['nama']) ?>" required>
            </div>
