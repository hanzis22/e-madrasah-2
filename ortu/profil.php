...
if (isset($_POST['simpan_profil'])) {
    ...
    $cek_username = mysqli_query($conn, "SELECT id FROM users WHERE username='$username_baru' AND id != '$id_ortu'");
    if (mysqli_num_rows($cek_username) > 0) {
        ...
        if (!empty($_FILES['foto']['name'])) {
            ...
            if (!in_array($tipe, ['jpg', 'jpeg', 'png', 'gif'])) {
                ...
                $nama_foto_baru = 'profil_' . $id_ortu . '_' . time() . '.' . $tipe;
...
// Ambil data ortu terbaru
 $data_ortu = mysqli_query($conn, "SELECT * FROM users WHERE id='$id_ortu'");
 $ortu = mysqli_fetch_assoc($data_ortu);
    ...
    <div class="box-putih">
        ...
        <form method="post" action="" enctype="multipart/form-data" style="max-width: 500px;">
            ...
            <div class="text-center">
                ...
                <input type="hidden" name="foto_lama" value="<?= $ortu['foto'] ?? '' ?>">
                <small class="text-muted d-block mb-4">Klik foto untuk mengubah. Maks 2MB</small>
            </div>

            <div class="mb-3">
                <label class="form-label">Username</label>
                <input type="text" name="username" class="form-control form-control-sm" value="<?= htmlspecialchars($ortu['username']) ?>" required>
                <small class="text-muted">Username digunakan untuk login.</small>
            </div>
            
            <div class="mb-3">
                <label class="form-label">Nama Lengkap</label>
                <input type="text" name="nama" class="form-control form-control-sm" value="<?= htmlspecialchars($ortu['nama']) ?>" required>
            </div>
